...
- 2026-05-31 全面 code review 发现的 22 个 bug 中，#1-#10 和 #22 已关闭；#17 的 XML attribute IP 脱敏已关闭但 secret/community attribute hardening 仍保留；#21 的旧碰撞描述已核实为过期。
- 当前没有已知 P0/P1 事务正确性或 recovery 崩溃安全 open bug；剩余约 10 个 confirmed-open 项均为 P3/deferred hardening，主要集中在 sample collector、YANG schema 边界和少量 adapter 输入一致性。
