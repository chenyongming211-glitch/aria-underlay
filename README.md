...
- PBR/BGP 功能线初版：H3C state parser 可输出 PBR/BGP read-only high-risk audit，offline H3C acceptance report 输出 `read_only_audits`、结构化 `touched_scope`、`write_decision=read_only`、blast radius 和 unsupported paths；BGP 审计已细化到 local AS、neighbor address、remote AS、session state、import/export policy 和 per-neighbor raw path，并可加载脱敏真实 running XML 样本做 `real_sample_audits` 校准。BGP intent / dry-run gated skeleton 已接入 domain/proto/ChangePlan：可以表达 BGP process、neighbor 和 route-policy reference；ChangePlan 输出 BGP neighbor -> route-policy dependency、missing route-policy evidence 和结构化拒绝原因。仍不支持 PBR/BGP adapter renderer 或真实写配置。
