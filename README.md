...
- PBR/BGP 功能线初版：H3C state parser 可输出 PBR/BGP read-only high-risk audit，offline H3C acceptance report 输出 `read_only_audits`、结构化 `touched_scope`、`write_decision=read_only`、blast radius 和 unsupported paths；仍不支持 PBR/BGP 写配置。
...
3. DeviceModelProfile、SoT Snapshot、ChangePlan dry-run 和 YANG schema collection 已落地；PBR/BGP read-only audit 和结构化 touched_scope 已接入 H3C parser/offline report，下一步用真实 running XML 样本校准 parser 并推进 path-level profile 验证，不直接写配置。
4. H3C Batch 2 Basic IPv4 ACL 仍是低风险命令扩展候选，但不应跳过 DeviceModelProfile/ChangePlan 基础；PBR/BGP 先做 read-only parser/audit，不直接写私有 renderer。
