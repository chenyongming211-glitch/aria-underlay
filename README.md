...
3. 在 PBR/BGP/QoS 等高风险写入前，先落地 DeviceModelProfile、SoT Snapshot 边界和 ChangePlan dry-run：先探测 OpenConfig/gNMI 或稳定 YANG path 级读写能力，再决定标准模型写入、厂商 YANG 写入或拒绝写入。
...
6. 真实交换机到位后，先采集 running XML、YANG/gNMI capabilities 和 model profile，再验证 parser 与 renderer 下发。
