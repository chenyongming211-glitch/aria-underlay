...
def _parse_real_interfaces(root, vlan_node, *, model_hint: str, scope) -> list[dict]:
    ...
    known_ifindexes = _ifmgr_ifindexes(root) | _vlan_interface_ifindexes(vlan_node)
    ...
    for ifindex, scoped_name in sorted(scope_names.items()):
        if ifindex not in known_ifindexes or scoped_name in seen:
            continue
...
def _vlan_interface_ifindexes(vlan_node) -> set[int]:
    ifindexes = set()
    for interface in _descendants(vlan_node, "Interface") if vlan_node is not None else []:
        ifindex_node = _first_child(interface, "IfIndex")
        if ifindex_node is None:
            continue
        try:
