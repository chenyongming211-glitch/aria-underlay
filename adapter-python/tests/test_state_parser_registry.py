...
def test_state_parser_registry_can_return_skeleton_when_explicitly_allowed(
    ...
    if isinstance(parser, H3cStateParser):
        assert parser.production_ready is True
    else:
...
def test_state_parser_registry_returns_h3c_parser_by_default(vendor):
    parser = state_parser_for_vendor(vendor, model_hint="S5560-54C-EI")

    assert isinstance(parser, H3cStateParser)
    assert parser.production_ready is True
