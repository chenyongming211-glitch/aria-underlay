...
Python 适配器 可以作为进程无状态服务运行，但事务产物不能丢失。
...
- 并发 apply 不能互相覆盖 desired state、journal 或 回滚工件。
...
- confirmed commit、回滚工件、running backup 等恢复所需信息必须持久化。
...
| Fail-closed | 多厂商适配没写完时，不能生成假 XML 或假成功 | 渲染器 / 驱动 未实现必须报错，不允许 模拟 数据进入生产路径 | 驱动 注册时检查能力；缺失方法返回 `Unimplemented` 或等价错误码；模拟 / fake 驱动 只能用于测试 配置档案 |
| Capability 驱动 | 不同交换机事务能力差异很大 | 根据设备 能力 选择策略；不支持强事务就显式 degraded 或失败 | 探测 `:candidate`、`:confirmed-commit`、`:rollback-on-error`、`:validate`、`writable-running`；降级必须返回 warning；记录 能力 快照，避免设备升级或替换后策略静默变化 |
...
| Recovery 可恢复 | 进程崩溃、session 断开后不能丢状态 | journal、回滚工件、confirmed-commit 信息必须持久化 | 启动时扫描 journal，根据 phase 执行 recover / cancel / verify；artifact 需要 checksum、retention、权限隔离，必要时压缩存储 |
...
| 凭据安全 | 私有化交付不能泄露设备密码 | inventory、journal、审计 只保存 `secret_ref`，不落明文密码 | 日志中任何敏感字段打印前必须脱敏；支持从外部 secret store 动态获取；journal / artifact 不得包含密码、私钥或 token |
...
| 测试优先 | 没真实交换机时更依赖 模拟 覆盖 | 模拟 adapter 必须覆盖 prepare / commit / verify / recover / rollback 失败路径 | 增加 crash / restart、session drop、timeout、adapter 返回 `InDoubt` 的混沌测试；真实设备阶段验证 能力 歧义、命名空间差异、CLI 降级 |
...
批量编排不承诺跨 endpoint “一起成功或一起回滚”。如果 MLAG 双 ToR 中 A 成功、B 失败，系统必须明确返回部分失败、记录 审计/journal，并允许 B 单独重试或由上层发起补偿操作。

Python 适配器 负责单设备事务动作：
...
适配器 不得只根据当前设备状态猜测恢复动作。对于 `candidate commit` 已进入 commit 阶段但无法证明 running 是否变更的情况，必须返回 `InDoubt`，不能假装回滚成功。
...
当设备不支持 NETCONF，或 NETCONF 实现不可用时，Python 适配器 必须保留降级通道：
...
该接口不是第一阶段强制实现项，而是 Aria Underlay 的长期目标。它的定位是让 Rust 主控在一个流式事务中向 Python 适配器 逐步发送 `Prepare`、`Commit`、`Verify`、`FinalConfirm`、`Abort` 等命令，同时 适配器 在同一个 stream 生命周期内维护 NETCONF session、candidate lock、confirmed-commit 上下文和厂商驱动状态。
...
- 适配器 在事务期间持有 candidate lock，避免多次握手和重复 lock/unlock。
...
- 适配器 必须校验 command 顺序，不允许乱序 commit 或重复 final confirm 造成状态错乱。
...
- `PinnedKey`：Rust/Python 边界传递 pinned fingerprint，但 Python ncclient
  ...
  失败关闭，返回 `HOST_KEY_PINNING_UNSUPPORTED`。
...
- 当调用方显式设置 `BlockNewTransaction` 时，如果设备 lifecycle 已被巡检标记为 `Drifted`，新的 apply 必须 失败关闭，返回 `DRIFT_BLOCKED`。
- `AutoReconcile` 只有在后续实现自动修复闭环后才能启用；当前遇到 `Drifted` 设备必须返回 `DRIFT_AUTORECONCILE_UNIMPLEMENTED`，不允许假装已修复。
- 后台巡检实现应以 shadow store 中的 expected state 为基准，通过 observation source 拉取 observed state，输出结构化 `DriftReport`；测试可继续使用静态 快照 输入。
...
| `Normal` | 默认生产路径 | 预检 使用 authoritative current，normalize 后 diff；post-commit verify 使用 touched scope | 默认 |
...
说明：在没有资源 ownership index / tombstone 之前，预检 不能只按 desired scope 读取。否则 desired 中已经删除的 VLAN/接口 不会被读到，diff 无法生成 delete 操作。第一阶段为了正确性，预检 保持全量或“Aria 已纳管资源集合”级别的权威读取；性能优化先落在 post-commit scoped verify。

第一阶段引入 `ShadowStateStore` 保存最近一次 refresh / 预检 / successful apply 后的结构化状态。shadow 可作为 漂移审计器 的 expected state，也可作为后续 Fast NoOp 的预判依据。
...
- 预检 diff 只有在存在资源 ownership index 时才能安全使用 scoped current。
- 适配器 不支持 scoped filter 时必须 失败关闭 或明确降级为 full refresh warning，不能静默返回假 scoped state。
...
- touched 接口 的 admin state / description / access / trunk 是否符合期望。
...
适配器 prepare 阶段必须把一次事务内的 VLAN 和 接口 变更合并为一次 `<edit-config>`：
...
不允许对同一个 endpoint 的多个 VLAN/接口 逐条调用多次 `edit-config`，除非厂商设备明确要求，并且必须记录性能 warning。
...
| 2 | Rust 从 desired/change-set 派生 scope | VLAN/接口 精确传给 适配器 |
| 3 | 适配器 支持 scoped state | 未实现真实 filter 时 失败关闭 或 full-refresh warning |
...
Transaction Journal 和 回滚工件 必须持久化，但不能无限增长。
...
适配器 只接收解析后的短期凭据或 密钥引用，不应长期保存明文密码。
