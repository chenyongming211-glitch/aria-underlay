...
| Operations | Audit/metrics still need production audit integration | Force-resolve, drift audit, GC, recovery, and transaction InDoubt events now map into service-queryable operation summaries and metrics counters. A JSONL file-backed `OperationSummaryStore` exists for restart-safe local persistence, with record-count/byte retention, archive rotation, fail-closed corrupt-record handling, daemon-scheduled compaction, `audit.write_failed` observability, service-level result filtering, aggregate overview counts, formal local `aria-underlay-ops` read commands, checked-in daemon config sample, operator runbook, internal JSONL operation alert storage with checkpointed dedupe, RBAC/product-audit enforcement for `force_resolve_transaction`, internal alert lifecycle actions, and local worker config retention/schedule changes. Remaining work is the real product audit database, identity provider integration, product API authorization, product UI/API packaging, and online daemon reload/product config management. External webhook, enterprise IM, PagerDuty, and email delivery are intentionally out of scope. | Keep privileged operation changes behind `AuthorizationPolicy` and `ProductAuditStore`; extend the same fail-closed pattern to product API operations and any future online reload path. |
...
| Drift | Drift auditor still needs deployment integration | One-shot audit, scheduler-facing summary, event emission, periodic worker entrypoint, shared `UnderlayWorkerRuntime`, the `aria-underlay-worker` JSON-configured daemon binary, and internal alert lifecycle triage now exist. Production still needs deployment packaging and system service ownership. | Add deployment packaging/system service ownership; keep AutoReconcile fail-closed until explicitly designed. |
...
1. Extend product audit/RBAC beyond local `aria-underlay-ops` flows to product
   API operations and any future online daemon reload path.
