...
| Operations | Audit/metrics still need production audit integration | Force-resolve, drift audit, GC, recovery, and transaction InDoubt events now map into service-queryable operation summaries and metrics counters. A JSONL file-backed `OperationSummaryStore` exists for restart-safe local persistence, with record-count/byte retention, archive rotation, fail-closed corrupt-record handling, daemon-scheduled compaction, `audit.write_failed` observability, service-level result filtering, aggregate overview counts, formal local `aria-underlay-ops` read commands, checked-in daemon config sample, operator runbook, internal JSONL operation alert storage with checkpointed dedupe, RBAC/product-audit enforcement for `force_resolve_transaction`, internal alert lifecycle actions, local worker config retention/schedule changes, `ProductOpsManager` RBAC gates for product-facing summary reads and product audit export, and `ProductOpsApi` handler-facing request/session facade with mock header extraction. Remaining work is the real product audit database, identity provider integration, real HTTP routing/server, product UI/API packaging, and online daemon reload/product config management. External webhook, enterprise IM, PagerDuty, and email delivery are intentionally out of scope. | Keep privileged operation changes behind `AuthorizationPolicy` and `ProductAuditStore`; replace mock header extraction with real IdP/session validation before exposing HTTP routes. |
...
2. Add real HTTP/product API routing on top of `ProductOpsApi`.
