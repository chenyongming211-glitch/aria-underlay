...
| Operations | Audit/metrics still need production audit integration | Force-resolve, drift audit, GC, recovery, and transaction InDoubt events now map into service-queryable operation summaries and metrics counters. A JSONL file-backed `OperationSummaryStore` exists for restart-safe local persistence, with record-count/byte retention, archive rotation, fail-closed corrupt-record handling, daemon-scheduled compaction, `audit.write_failed` observability, service-level result filtering, aggregate overview counts, and offline `transaction_ops` read commands. Remaining work is the product audit backend, authorization model, alert integration, and formal UI/CLI packaging. | Use `JsonFileOperationSummaryStore` where local file persistence is acceptable; wire the same `OperationSummaryStore` trait into the chosen production audit store once storage requirements are fixed. Route `UnderlayAuditWriteFailed` and attention-required summaries into the product alerting path when that backend exists. |
...
1. Add product audit backend, authorization, formal CLI/UI packaging, and alert
   delivery on top of `OperationSummaryStore` once storage and authorization
