...
| Operations | Audit/metrics still need production audit integration | Force-resolve, drift audit, GC, recovery, and transaction InDoubt events now map into service-queryable operation summaries and metrics counters. A JSONL file-backed `OperationSummaryStore` exists for restart-safe local persistence. The remaining work is the product audit backend, UI/CLI surface, retention policy, and authorization model. | Use `JsonFileOperationSummaryStore` where local file persistence is acceptable; wire the same `OperationSummaryStore` trait into the chosen production audit store once storage requirements are fixed. |
| GC | GC still needs external deployment integration | `run_once`, retention policy, periodic worker entrypoint, event emission, deletion summaries, and a shared `UnderlayWorkerRuntime` lifecycle wrapper now exist. Production still needs wiring into the actual daemon binary/config, persistent audit sink, and any site-specific disk quota policy. | Mount `UnderlayWorkerRuntime` from the production process once daemon binary/config ownership is fixed. |
| Drift | Drift auditor still needs daemon integration | One-shot audit, scheduler-facing summary, event emission, periodic worker entrypoint, and a shared `UnderlayWorkerRuntime` lifecycle wrapper now exist. Production still needs wiring into the actual daemon binary/config and alert delivery path. | Mount `UnderlayWorkerRuntime` from the production process once daemon binary/config ownership is fixed; keep AutoReconcile fail-closed until explicitly designed. |
...
1. Wire `UnderlayWorkerRuntime` into the actual daemon binary/config once
   production process ownership is fixed.
2. Add product audit backend/UI/CLI integration on top of `OperationSummaryStore`
