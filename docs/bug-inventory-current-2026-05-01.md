...
| --- | --- | --- | --- |
| Transactions | Crash/restart matrix is still incomplete | The protocol is much stronger now, but more file-backed restart cases are needed to prove recovery behavior around terminal and manual-resolution phases. | Add tests for journal residue, shadow failure, `ForceResolved` restart, and terminal `Committed` / `Failed` / `RolledBack` reload behavior. |
