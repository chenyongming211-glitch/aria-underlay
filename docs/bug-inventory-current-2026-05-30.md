# 当前缺陷 / 技术债清单 — 2026-05-30

> 当前有效基线。本文在 `docs/bug-inventory-current-2026-05-09.md`
> 基础上，核实 gRPC TLS/mTLS 修复并更新分类。旧文件保留审查证据和
> 历史上下文，但不要再直接作为 open bug 队列使用。
>
> 2026-06-05 更新：本文已被 `docs/bug-inventory-current-2026-06-05.md`
> 取代。本文只保留 2026-05-30 前缺陷清零的历史基线。

## 核实基线

- 代码：`main` / 2026-05-30 TLS fix
- 本地验证：`python3 -m pytest -q adapter-python/tests` -> `310 passed`
  （含 13 个新增 TLS/mTLS 集成测试）
- GitHub Actions：Rust `cargo check` / `cargo test` 结果以 CI 为准。
...
| NETCONF force unlock 未实现 | Python real NETCONF backend 现在校验 `lock_owner` 必须是正整数 NETCONF session-id，并调用 ncclient `kill_session(session_id)`；RPC 失败映射为 `NETCONF_FORCE_UNLOCK_FAILED`，非法 session-id 映射为 `NETCONF_FORCE_UNLOCK_SESSION_ID_INVALID`。 | `adapter-python/aria_underlay_adapter/backends/base.py`, `adapter-python/aria_underlay_adapter/backends/netconf.py`, `adapter-python/aria_underlay_adapter/drivers/netconf_backed.py`, `adapter-python/tests/test_netconf_backend.py` | 已通过 GitHub Actions run `25619885493`；本地 Python adapter 当前 `297 passed`，新增 force-unlock 成功、非法 session-id 和 kill-session 失败映射回归测试。 |
| `_persist_id_already_consumed` 字符串 fallback | confirmed-commit recovery 现在只接受结构化 `AdapterError.code` 或 `normalized_error` 表示 persist-id 已消费/不存在，不再从 `message` 或 `raw_error_summary` 中按 `"persist"`、`"unknown"`、`"not found"`、`"consumed"` 猜测。 | `adapter-python/aria_underlay_adapter/drivers/netconf_backed.py`, `adapter-python/tests/test_netconf_backend.py` | 本地 Python adapter 当前 `297 passed`；新增文本 fallback 不再误判 committed、结构化 code/normalized_error 仍识别 committed 的回归测试。 |

## Fixed in current branch — 2026-05-30

| 项目 | 当前确认 | 主要证据 | 验证状态 |
| --- | --- | --- | --- |
| Python Adapter gRPC TLS/mTLS | Python adapter server 现在支持 TLS 和 mTLS：当 `ARIA_UNDERLAY_ADAPTER_TLS_CERT_FILE` 和 `ARIA_UNDERLAY_ADAPTER_TLS_KEY_FILE` 配置时，使用 `add_secure_port` 而非 `add_insecure_port`；可选 `ARIA_UNDERLAY_ADAPTER_TLS_CA_CERT_FILE` 启用 mTLS 客户端证书验证。Rust `AdapterClientPool` 新增 `TlsConfig` 和 `with_tls()` 方法，为 `https://` endpoint 自动配置 `ClientTlsConfig`（含 CA 证书和客户端 Identity）。 | `adapter-python/aria_underlay_adapter/config.py`（`tls_cert_file` / `tls_key_file` / `tls_ca_cert_file` 字段、`tls_enabled` / `mtls_enabled` 属性、`validate()` 方法），`adapter-python/aria_underlay_adapter/server.py`（`build_server_credentials()`、`build_server()`），`src/adapter_client/client.rs`（`TlsConfig`、`AdapterClientPool::with_tls()`、`client()` 中 TLS 配置应用），`tests/adapter_client_pool_tests.rs`（TLS pool 测试） | 本地 Python adapter `310 passed`；新增 13 个 TLS 测试覆盖：config 解析、validation、credential 构建、insecure/TLS/mTLS server 端到端集成（含 insecure client 被 TLS server 拒绝、无证书 client 被 mTLS server 拒绝、可信 client 成功连接）。Rust `cargo check` / `cargo test` 以 CI 为准。 |

## Confirmed-open

无已知非条件 open bug。当前部署边界下所有已确认缺陷均已修复。
