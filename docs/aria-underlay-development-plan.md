...
- fail-closed 不是简单返回错误，而是要给出结构化错误码、归一化错误、原始摘要和 retryable 语义。
- 能在无真实交换机环境验证的部分必须先通过 fixture、fake backend、snapshot 或 file-backed 测试闭环。
- 不能因为当前没有真实设备就降低 production gate；fixture/skeleton 能验证框架，但不能冒充生产可用。
