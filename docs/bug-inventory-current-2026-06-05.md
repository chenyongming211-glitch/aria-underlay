...
- 当前无 confirmed-open bug。PBR/BGP 写配置仍需真实设备 path-level evidence，
  未验真前保持 read-only / write-rejected。
...
| #11 | `_port_mode_to_proto` 不接受数值 `kind` | 已修复 | `test_port_mode_accepts_numeric_proto_kind_values` |
| #12 | YANG namespace 只搜索前 2000 字符 | 已修复 | `test_collect_yang_schemas_extracts_namespace_beyond_short_preamble` |
| #13 | `collect-device-sample --password` 暴露在进程列表 | 已修复 | `test_cli_rejects_password_argument`; CLI 使用 `--password-env` 或交互输入 |
| #14 | sample collector 默认跳过 host-key verify | 已修复 | `test_device_collection_verifies_hostkey_by_default`, `test_device_collection_requires_explicit_warning_for_skipped_hostkey` |
...
| #18 | `_safe_path_component` 不过滤 `..` | 已修复 | `test_safe_path_component_removes_parent_directory_segments` |
| #19 | `confirm_timeout_secs=0` 传入 Python NETCONF commit 时被替换为 120 | 已修复 | `test_confirmed_commit_rejects_zero_timeout_in_driver`, `test_commit_candidate_rejects_zero_confirm_timeout_before_touching_device` |
| #20 | `load_yang_library` 文件缺失时状态不一致 | 已修复 | `test_load_yang_library_marks_missing_downloaded_schema_as_skipped` |
...
1. 功能开发上仍建议继续 BGP 相关路线：先做真实 H3C BGP running XML 样本校准和 path-level profile evidence，不直接做 BGP 写配置 renderer。
2. 真实设备到位前，继续保持 PBR/BGP 写配置 read-only / write-rejected，不把离线 parser evidence 等同于生产写入证据。
