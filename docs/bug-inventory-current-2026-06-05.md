...
- 已关闭：#1-#10、#12-#18、#20、#22。
- 旧描述已不准确：#21 设备名匿名化不再是 10000 空间；当前实现使用 24-bit hash
  输出空间，原“约 120 台设备 50% 碰撞概率”不成立。
- 仍待修：2 个 deferred bug/hardening 项，均不阻塞当前 PBR/BGP read-only、
  H3C ACL 验收和事务主路径。
...
1. 第四批修 adapter 输入一致性：#11、#19。
2. 功能开发上仍建议继续 BGP 相关路线：先做真实 H3C BGP running XML 样本校准和 path-level profile evidence，不直接做 BGP 写配置 renderer。
