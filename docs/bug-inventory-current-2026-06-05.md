...
- 已关闭：#1-#10、#22。
- 部分关闭：#17 的 XML attribute IP 脱敏已关闭；attribute 中的 password/community
  string 仍作为低优先级 hardening 保留。
- 旧描述已不准确：#21 设备名匿名化不再是 10000 空间；当前实现使用 24-bit hash
  输出空间，原“约 120 台设备 50% 碰撞概率”不成立。
- 仍待修：10 个 deferred bug/hardening 项，均不阻塞当前 PBR/BGP read-only、
  H3C ACL 验收和事务主路径。
...
| 编号 | 当前状态 | 说明 |
| --- | --- | --- |
| #17 | 部分修复 | XML attribute 中的 IP 已脱敏，见 `test_ip_in_attributes`。但 attribute 名称暗示 password/community 时，当前仍只做 IP 替换，不会按 secret 字段整值 redaction；作为 open hardening 继续保留。 |
...
| #15 | P3 | `--from-file` 非 UTF-8 / binary / malformed XML 仍可能 raw traceback | from-file 路径直接 `read_text()` 后 `sanitize_xml()`。 | 捕获 `UnicodeDecodeError` / `ValueError`，输出用户友好错误。 |
| #16 | P3 | `--from-file` 和 `--output` 同路径会覆盖原始文件 | 当前没有 same-path guard。 | 若输入输出解析为同一路径则 fail-closed。 |
| #17-remain | P3 | XML attribute secret/community 字段未整值 redaction | attribute 值当前只经过 IP regex 替换。 | 对 attribute name 命中 password/community/key/secret 时 redaction。 |
...
1. 先修 sample collector / YANG schema 中最小、低风险、和真实样本采集直接相关的 hardening：#15、#16、#17-remain、#18、#20。
2. 再处理 #12，因为 YANG schema collection 会支撑后续 OpenConfig / native YANG path-level profile。
3. #11、#13、#14、#19 可后置，除非要立刻把 sample collector 当成多人/生产环境工具使用。
